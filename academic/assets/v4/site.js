...
document.querySelectorAll('.filter-bar').forEach(bar=>{
 ...
 function apply(){let count=0;const q=(search?.value||'').trim().toLocaleLowerCase();[...list.children].forEach(item=>{const kinds=(item.dataset.kind||'').split(/\s+/),years=(item.dataset.year||item.querySelector('.paper-year')?.firstChild.textContent.trim()||'').split(/\s+/);item.hidden=(category!=='all'&&!kinds.includes(category))||(q&&!item.textContent.toLocaleLowerCase().includes(q))||(year&&year.value!=='all'&&!years.includes(year.value));if(!item.hidden)count++});buttons.forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.filter===category)));result.textContent=`${count} ${result.dataset.unit}`;bar.closest('section')?.querySelector('.empty')?.toggleAttribute('hidden',count!==0)}
